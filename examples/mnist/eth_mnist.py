...
parser.add_argument("--time", type=int, default=160)
...
wandb.init(project="snn_16bit", config={"n_epochs": n_epochs,
                               "n_neurons": n_neurons,
                               "exc": exc,
                               "inh": inh,
...
for epoch in range(n_epochs):
    ...
    for step, batch in enumerate(tqdm(dataloader)):
        ...
        if step % update_interval == 0 and step > 0:
            ...
            wandb.log({"Accuracy_train": np.max(accuracy["all"])})
            print(
...
wandb.log({"Accuracy_test": (accuracy["all"] / test_dataset.test_labels.shape[0])})
